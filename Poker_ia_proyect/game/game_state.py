...
class GameState:
    ...
    def __init__(self, player1: Player, player2: Player, small_blind: int, big_blind: int):
        ...
        self.stack_player = None
        self.stack_opponent = None
        self.pot = None
    ...
    def update_from_vision(self, data: dict):
        # Asume keys: stack_player, stack_opponent, pot, etc. Mantiene lógica desacoplada.
        if data.get('stack_player') is not None:
            self.stack_player = data['stack_player']
        if data.get('stack_opponent') is not None:
            self.stack_opponent = data['stack_opponent']
        if data.get('pot') is not None:
            self.pot = data['pot']
        if 'dealer_button' in data and data['dealer_button'] is not None:
            self.dealer_button = data['dealer_button']
        # Otros campos se asignan aquí según el proyecto

    def __repr__(self):
        return f"<GameState stack_player={self.stack_player} stack_opponent={self.stack_opponent} pot={self.pot} dealer_button={self.dealer_button}>"
