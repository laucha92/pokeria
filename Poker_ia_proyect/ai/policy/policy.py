...
class Policy:
    ...
    def __init__(
        ...
        Args:
            model (nn.Module): Modelo principal para la selección de acciones.
            mcts_agent (MCTS_Agent): Agente MCTS para la selección de acciones.
            cfr_data (Optional[dict]): Datos CFR para la selección de acciones.
            strategy_weights (Optional[Dict[str, float]]): Pesos iniciales para las estrategias.
            selection_mode (str): Modo de selección de acciones ('argmax', 'sampling', 'epsilon_greedy').
            epsilon (float): Valor inicial de epsilon para epsilon-greedy.
            temperature (float): Valor inicial de temperatura para sampling.
            debug_mode (bool): Modo de depuración.
            adaptive_weights (bool): Indica si se deben adaptar los pesos de las estrategias.
            state_size (int): Tamaño del estado del juego.
            game_context_analyzer (Optional[GameContextAnalyzer]): Analizador del contexto del juego.
            use_confidence (bool): Indica si se debe usar la confianza de las estrategias.
            initial_temperature (float): Temperatura inicial para el muestreo de acciones.
            temperature_decay (float): Factor de decaimiento de la temperatura.
            min_temperature (float): Temperatura mínima.
            epsilon_decay (float): Factor de decaimiento de epsilon.
            min_epsilon (float): Epsilon mínimo.
            fallback_policy (Optional[FallbackPolicy]): Política de fallback.
            external_logging_callback (Optional[Callable]): Callback externo para el logging.
            replay_mode (bool): Modo de repetición.
            use_cache (bool): Indica si se debe usar el cache.
            cache_size (int): Tamaño del cache.
            model_save_path (str): Ruta para guardar el modelo.
            initial_confidence_threshold (float): Umbral de confianza inicial para el voting/ranking.
            confidence_threshold_decay (float): Factor de decaimiento del umbral de confianza.
            min_confidence_threshold (float): Umbral de confianza mínimo.
            pca_components (int): Número de componentes PCA a utilizar.
            heuristic_fallback_policy (Optional[HeuristicFallbackPolicy]): Política de fallback heurística.
            use_weighted_voting (bool): Indica si se debe usar el voto ponderado en lugar de corte por umbral.
            trust_decay_factor (float): Factor de decaimiento del trust a lo largo del tiempo.
            min_trust (float): Trust mínimo.
            cache_invalidation_threshold (float): Umbral para invalidar el cache.
            training_mode (bool): Indica si se está en modo de entrenamiento.
            exploration_factor (float): Factor de exploración durante el entrenamiento.
        """
        self.model = model
    ...
    def select_action(self, game_state: List[float], round_state: Dict) -> int:
        ...
        try:
            ...
            raise PolicyActionSelectionError(f"No se pudo seleccionar la acción") from e
