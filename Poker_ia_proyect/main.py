...
from utils import logger, data_utils, config_utils, save_game_history
...
def obtener_frame():
    cap = cv2.VideoCapture(0)  # Cámara 0 fija
    if not cap.isOpened():
...
def main():
    ...
    chip_detector = ChipDetector(visual_debug=False)
    player1, player2, game_state = initialize_game(config)
    ...
    try:
        ...
        print(f"Error al cargar parámetros de cámara: {e}")
        sys.exit(1)

    # Instanciar detectores y estado de juego
    card_detector = CardDetector()
    chip_detector = ChipDetector(visual_debug=False)
    vision_system = VisionSystem()
    game_state = GameState()

    # Abrir cámara UNA sola vez y mantenerla abierta
    ...
    try:
        while True:
            ...
            cards = card_detector.detect_and_recognize(frame)
            chips = chip_detector.detectar_informacion(frame)
            game_state_vision = vision_system.get_game_state(frame)

            # Actualizar el estado del juego con la info visual detectada
            game_state.update_from_vision(game_state_vision)

            # Mostrar por consola (o loguear) la info relevante
            print("Cartas detectadas:", cards)
            print("Fichas detectadas:", chips)
            print("Estado visión:", game_state_vision)
            print("Estado juego actualizado:", game_state)

            # Aquí podrías llamar a la IA o lógica que tome decisiones con game_state
