...
def obtener_frame():
    # Adaptar según método de captura
    img = cv2.imread("screenshot_poker.png")
    if img is None:
        raise ValueError("No se pudo capturar la pantalla")
    return img
...
def main():
    ...
    while True:
        try:
            frame = obtener_frame()
            datos_vision = chip_detector.detectar_informacion(frame)
            game_state.update_from_vision(datos_vision)
            print(game_state)
            # Aquí van las llamadas a la IA/MCTS/CFR/Redes, usando game_state
        except Exception as e:
